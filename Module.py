...
def Power_System_Model(Model):
    ...
    if Setting.CCS_allowed==False and Setting.expansion_allowed:
    ...
    # cost components
    ccs_cost=0;est_cost=0;dec_cost=0;fom_cost=0;
    if Setting.expansion_allowed:
        ...
        fom_cost = LinExpr(quicksum(Plants[i].FOM*Enodes[n].Init_plt_count[i] for n in range(nE) for i in range(nPlt)));
    ...
    if Setting.emis_case==1: # add gas fuel cost only if Case=0 (power system only)
        e_total_cost = gas_fuel_cost+est_cost+dec_cost+fom_cost+vom_cost+nuc_fuel_cost+ccs_cost+startup_cost+shed_cost+strg_cost1+tran_cost+trans_Fom;
    else:
        e_total_cost = est_cost+dec_cost+fom_cost+vom_cost+nuc_fuel_cost+ccs_cost+startup_cost+shed_cost+strg_cost1+tran_cost+trans_Fom;
    ...
    if Setting.expansion_allowed:
        ...
        Model.addConstrs(EV.prod[n,t,i]<= Plants[i].nameplate_cap*Enodes[n].Init_plt_count[i] for n in range(nE) for i in range(nPlt) for t in Te if (plant2sym[i] in thermal_units))
    if Setting.UC_active:
        # UC:
        ...
        if Setting.expansion_allowed:
            ...
            Model.addConstrs(EV.X[n,t,i] <= Enodes[n].Init_plt_count[i] for n in range(nE) for i in range(nPlt) for t in Te if (plant2sym[i] in thermal_units));
    else:
        if Setting.expansion_allowed:
            ...
            Model.addConstrs(EV.prod[n,t,i]-EV.prod[n,t-1,i]<=Plants[i].ramp_rate* Plants[i].nameplate_cap*Enodes[n].Init_plt_count[i] for n in range(nE) for i in range(nPlt) for t in Te if (t>0 and plant2sym[i] in thermal_units));
            Model.addConstrs(-EV.prod[n,t,i]+EV.prod[n,t-1,i]<=Plants[i].ramp_rate* Plants[i].nameplate_cap*Enodes[n].Init_plt_count[i] for n in range(nE) for i in range(nPlt) for t in Te if (t>0 and plant2sym[i] in thermal_units));
    ...
    if Setting.expansion_allowed:
        ...
        Model.addConstrs(EV.prod[n,t,i]<=Enodes[n].cap_factors[e_rep_hrs[t],i]* Plants[i].nameplate_cap*Enodes[n].Init_plt_count[i] for n in range(nE) for i in range(nPlt) for t in Te);
    ...
    if Setting.expansion_allowed:
        ...
        Model.addConstrs(0>= EV.eSdis[n,t,r] for n in range(nE) for r in range(neSt) for t in Te if t>0);
        Model.addConstrs(0>= EV.eSch[n,t,r] for n in range(nE) for r in range(neSt) for t in Te if t>0);
        Model.addConstrs(0>= EV.eSlev[n,t,r] for n in range(nE) for r in range(neSt) for t in Te if t>0);
...
def NG_System_Model(Model): # for the full year
    ...
    if Setting.relax_int_vars==False and Setting.expansion_allowed:
    ...
    if Setting.expansion_allowed:
        ...
        pipe_FOM = LinExpr(quicksum(PipeLines[b].FOM*PipeLines[b].length*PipeLines[b].is_exist for b in range(nPipe)));  
        
    pipe_Dec=0;
    if Setting.expansion_allowed:
        pipe_Dec = LinExpr(quicksum(PipeLines[b].decom*PipeLines[b].length*GV.ZgDec[b] for b in range(nPipe)));  
    
    if Setting.LCF_supply_curve==1 and Setting.expansion_allowed:
    ...
    if Setting.expansion_allowed:
        ...
        Model.addConstrs(GV.flowGG[i,tau]<=PipeLines[i].Cap*PipeLines[i].is_exist for i in range(nPipe) for tau in FY);
    ...
    # else:
    #     Model.addConstrs(GV.ZgOp[b] == PipeLines[b].is_exist for b in range(nPipe) if PipeLines[b].is_exist==1);
    #     Model.addConstrs(GV.ZgOp[b] == 0 for b in range(nPipe) if PipeLines[b].is_exist==0);
    ...
    for l in range(len(dfd)):
        Model.addConstrs(GV.flowGG[l,tau]<=(1-dfd['disruption_rate'].iloc[l])*PipeLines[l].Cap*PipeLines[l].is_exist for tau in range(dfd['from_day'].iloc[l], dfd['to_day'].iloc[l]));
    ...
    for _, row in dfd.iterrows():
        ...
        if Setting.expansion_allowed:
            ...
            Model.addConstrs(GV.flowGG[l,tau]<=(1-dr)*PipeLines[l].Cap*PipeLines[l].is_exist for tau in range(fr_day, to_day+1));
...
def Coupling_constraints(Model):
    ...
    if Setting.emis_case==4:  #global emission limit
        if Setting.emis_reduc_goal>0.01:
            Model.addConstr(EV.emis_amount+GV.emis_amount<=(1-Setting.emis_reduc_goal)*Setting.CO2_emission_1990);
...
def Publish_results(s_time,MIP_gap):
    ...
    header0 = ['Power_network_size','cluster_method','Base_Year',
              ...
              'UC-rlx?','int-vars-rlx?','copper_plate?',
              'CCS-allowed?',
              ...
              'storage1-cost','tran-est-cost','trans_FOM','CCS-cost',
    ...
    if Setting.print_all_vars:
        ...
        for g in range(nG): header.append(str(g));     
        row0 = [['']*(len(header)+10) for i in range(365*24)];
